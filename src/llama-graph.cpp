...
llm_graph_context::llm_graph_context(const llm_graph_params & params) :
    ...
    // Disk-offloaded expert banks are metadata-only in the model. Keep the real top-k during
    // warmup so the graph still exercises the interceptor path (256 experts would overflow both
    // the layer ring and the gate mailbox ids).
    n_expert_used    (params.moe_prefill ? hparams.n_expert_used :
                      (cparams.warmup ? hparams.n_expert : hparams.n_expert_used)),
    ...
    moe_prefill      (params.moe_prefill),
    cvec             (params.cvec),
    loras            (params.loras),
    mctx             (params.mctx),
    cross            (params.cross),
